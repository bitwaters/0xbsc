...
export function trialReport(db: SqliteDatabase, runId: string) {
  const run = db
    .prepare('SELECT manifest_json,status FROM research_runs WHERE run_id=?')
    .get(runId) as { manifest_json: string; status: string } | undefined;
  ...
  const reasons = db
    .prepare(
      "SELECT json_extract(metadata_json,'$.reason') AS reason,COUNT(*) AS count FROM operation_traces WHERE stage='trial_funnel' AND json_extract(metadata_json,'$.runId')=? GROUP BY reason ORDER BY count DESC"
    )
    .all(runId);
